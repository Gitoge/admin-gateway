...
export class ActesComponent implements OnInit {
  ...
  trackId(_index: number, item: IActes): number {
    return item.id!;
  }

  delete(actes: IActes): void {
    const modalRef = this.modalService.open(ActesDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: IActes[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
