...
} from 'app/entities/carriere/situation-administrative/situation-administrative.model';
import { SituationAdministrativeService } from 'app/entities/carriere/situation-administrative/service/situation-administrative.service';
import { IBaremeCalculHeuresSupp } from '../../bareme-calcul-heures-supp/bareme-calcul-heures-supp.model';
import { BaremeCalculHeuresSuppService } from '../../bareme-calcul-heures-supp/service/bareme-calcul-heures-supp.service';

@Component({
  selector: 'jhi-saisie-elements-variables-update',
  templateUrl: './saisie-elements-variables-update.component.html',
})
export class SaisieElementsVariablesUpdateComponent implements OnInit {
  isSaving = false;
  isAjout!: boolean;
  posteId: any;
  ...
  constructor(
    ...
    protected heursuppService: BaremeCalculHeuresSuppService
  ) {}
  ...
  ngOnInit(): void {
    this.activatedRoute.data.subscribe(({ saisieElementsVariables }) => {
      if (saisieElementsVariables.id === undefined) {
        ...
        this.titre = 'Ajouter Elments';
      } else {
        this.titre = 'Modifier Elements';
      }
      this.isAjout = false;
  ...
  addHeuresSupp(): void {
    ...
    this.modalRef.componentInstance.agent = this.agent;
    this.modalRef.componentInstance.situationAdminisrtativeAgent = this.situationAdminisrtativeAgent;
    this.modalRef.componentInstance.findBaremeHeureSupp = this.findBaremeHeureSupp(this.situationAdminisrtativeAgent);
    this.modalRef.componentInstance.clickEvent.subscribe((heuresSupp: IHeuresSupp) => {
      if (this.findBaremeHeureSupp(this.situationAdminisrtativeAgent)) {
        this.montantHeurSupp =
          heuresSupp.dimanchesJoursFeries! * this.findBaremeHeureSupp(this.situationAdminisrtativeAgent).dimanchesJoursFeries! +
          heuresSupp.heuresOrdinaires! * this.findBaremeHeureSupp(this.situationAdminisrtativeAgent).heuresOrdinaires! +
          heuresSupp.heuresNuit! * this.findBaremeHeureSupp(this.situationAdminisrtativeAgent).heuresNuit!;
        const elt = new SaisieElementsVariables();
        const today = dayjs().startOf('day');
        elt.dateInsert = today;
        elt.matricule = this.matricule;
        elt.agentId = this.agentId;
        elt.montant = this.montantHeurSupp;
        elt.dateEcheance = this.periodePayeEnCours.dateFin;
        elt.date = this.periodePayeEnCours.dateDebut;
        elt.codePoste = heuresSupp.codePoste;
        elt.sens = this.sens;
        elt.posteId = this.posteId;
        elt.libellePoste = heuresSupp.libellePoste;
        elt.destinataire = this.codePosteDestinataire;
        this.elementsVar?.push(elt);
      }
    });
  ...
  findBaremeHeureSupp(situationAdministraive: ISituationAdministrative): IBaremeCalculHeuresSupp {
    if (situationAdministraive.indice != null) {
      this.heursuppService.findBaremeHeureSuppIndice(situationAdministraive.indice.soldeIndiciaire!).subscribe(
        (result: any) => {
          this.baremeCalculHeurSupp = result.body;
        },
        error => {
          Swal.fire({
            icon: 'error',
    ...
    if (situationAdministraive.soldeGlobal != null) {
      this.heursuppService.findBaremeHeureSuppSoldeGlobale(situationAdministraive.soldeGlobal).subscribe(
        (result: any) => {
          this.baremeCalculHeurSupp = result.body;
        },
        error => {
    ...
    return this.baremeCalculHeurSupp;
  }
  ...
  getAgentByMatricule(matricule: any): void {
    this.agentService.findAgentByMatricule(matricule).subscribe((result: any) => {
      ...
      this.getSituationAdministrativeAgent(this.agentId);
    });
  }

  getSituationAdministrativeAgent(agent: number): void {
    this.situationAdministrativeService.findByAgent(agent).subscribe(
      (result: any) => {
        this.situationAdminisrtativeAgent = result.body;
      },
      error => {
        Swal.fire({
          icon: 'error',
          title: 'Erreur...',
          text: 'Situation Introuvable introuvable!',
        });
      }
    );
  }
  ...
  protected onSaveSuccess(saisieElementsVariables: ISaisieElementsVariables | null): void {
    if (saisieElementsVariables !== null) {
      ...
      if (this.saisieElementsVariables.id !== undefined) {
        ...
        this.router.navigate(['/saisie-elements-variables/new']);
      }
      this.router.navigate(['/saisie-elements-variables/new']);
    }
  }
