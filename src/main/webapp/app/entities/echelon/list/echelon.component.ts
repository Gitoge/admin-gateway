...
export class EchelonComponent implements OnInit {
  ...
  constructor(protected echelonService: EchelonService, protected modalService: NgbModal, protected parseLinks: ParseLinks) {
    this.echelons = [];
  ...
  loadAll(): void {
    ...
    this.echelonService
      ...
      .subscribe({
        next: (res: HttpResponse<IEchelon[]>) => {
          this.isLoading = false;
          this.paginateEchelons(res.body, res.headers);
        },
        error: () => {
          this.isLoading = false;
        },
  ...
  trackId(_index: number, item: IEchelon): number {
    return item.id!;
  }
  ...
  protected paginateEchelons(data: IEchelon[] | null, headers: HttpHeaders): void {
